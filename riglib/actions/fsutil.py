...
def backup_path(target: Path) -> Path:
    ...
    bak = target.with_name(f"{target.name}.rig-bak-{stamp}")
    n = 1
    while bak.exists():
        bak = target.with_name(f"{target.name}.rig-bak-{stamp}.{n}")
        n += 1
    return bak
...
def copy_tree(source: Path, target: Path, on_conflict: str) -> WriteOutcome:
